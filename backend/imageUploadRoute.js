...
const cleanupFile = async(filePath) => {
    if(filePath){
        try{
            ...
            // It's okay if cleanup fails, but log the error.
...
router.post('/scan', upload.single('inventoryImage'), async (req, res) => {
    ...
    if (!req.file) {
        return res.status(400).json({ error: 'No image file uploaded. Please include a field named "inventoryImage".' });
    }
    ...
    try {
        ...
        // 4. Send the raw results back to the client (Task 1.4 will process this later)
        res.status(200).json({
            message: 'Image successfully analyzed by AI. Results below are raw.',
            // The visionResult structure is { labels, texts, cleanText }
            ...visionResult, 
        });
